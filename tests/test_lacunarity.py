...
def test_reproducibility():
    ...
    calc1, calcs1 = first_lacunarity.calc_all(
        os.path.join(get_project_path(), "example_data", "text_data.txt")
    )
    calc2, calcs2 = second_lacunarity.calc_all(
        os.path.join(get_project_path(), "example_data", "text_data_1.txt")
    )
    ...
    assert np.array_equal(
        calcs1["relationship_derivatives"], calcs2["relationship_derivatives"]
    )
