...
def plot_results(data, save_path) -> None:
    ...
    plt.plot(
        data["exp_lambdas"], data["exp_half_regressions"], marker="o", linestyle="-"
    )
    plt.title("Для каждого среза")
    ...
    plt.plot(
        data["pressures"], data["ex_ones_square"], label="Площадь внешних островов"
    )
    plt.plot(
        data["pressures"], data["in_ones_square"], label="Площадь вложенных островов"
    )
    ...
    plt.title(
        "Отношение площади вложенных 0 к общей площади на каждом срезе (Интегральные координаты)"
    )
    plt.xlabel("%, Срезания")
    ...
    plt.title(
        "Отношение площади вложенных 0 к общей площади на каждом срезе (Дифференциальные координаты)"
    )
    plt.xlabel("%, Срезания")
    plt.ylabel("dy/dx")
    plt.grid(True)
